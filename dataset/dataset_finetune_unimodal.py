...
from transformers import BertTokenizer, T5Tokenizer, AutoTokenizer, LongT5Model
...
def process_video_meta(data):
    ...
    for vid, data_item in data.items():
        ...
        #                                         data_item['sentences']):
        for sentence, summary, figs, gold_figs in zip(data_item["text"], data_item['abstract'],
            ...
            ab_sents_img_ind = ab_sents + "img_ind_" + str(gold_figs.index(1) if 1 in gold_figs else 0)

            img_ind = "img_ind_" + str(gold_figs.index(1) if 1 in gold_figs else 0)
            ...
            record = {'vid': str(vid), 'figs': figs, 'gold_figs': gold_figs, 'img_ind': img_ind, 
                      'tgt_img_ind': ab_sents_img_ind, 'tgt_img': tgt_img, 'src': sents, 
                      'tgt': ab_sents, 'src_summary': src_summary}
            results[idx] = record
            idx += 1
    return results
...
class gitDataset(BaseDataset):
    def __init__(self, data_path, split, args_config, encoder=False, task=None, transform=None):
        ...
        # TODO: new added ####################################################################################################
        self.label_tokenizer = AutoTokenizer.from_pretrained("google/pegasus-pubmed",
                                                             cache_dir='../../preTrain_model/google/pegasus-pubmed/')

        # self.label_tokenizer = AutoTokenizer.from_pretrained("allenai/led-base-16384",
        #                                                    cache_dir='preTrain_model/allenai/led-base-16384/')

        # self.label_tokenizer = AutoTokenizer.from_pretrained("google/long-t5-local-base",
        #                                                    cache_dir='preTrain_model/google/long-t5-local-base/')

        # self.label_tokenizer = AutoTokenizer.from_pretrained("allenai/scibert_scivocab_uncased",
        ...
        self.tokenizer = BertTokenizer.from_pretrained("allenai/scibert_scivocab_uncased",
                                                       cache_dir='../../preTrain_model/scibert_scivocab_uncased/')
        new_tokens = [f"img_ind_{_ind}" for _ind in range(20)]
        self.label_tokenizer.add_tokens(new_tokens)
        ...
        super(gitDataset, self).__init__()
    ...
    def _load_data(self):
        ...
        for _, row in self.data.items():  # get data from files
            ...
            self.summary.append(row["tgt"])
            self.text.append(row["src"])
            self.img_ind.append(row['img_ind'])
            self.captions_img_ind.append(row['tgt_img_ind'])
            self.gold_figs.append(row['gold_figs'])

            if "image" in self.modality:
                img_file2d = f'{self.args_config.data_path}/{self.args_config.data_name}/images_feat/' + \
                             row["vid"] + '.pkl'
                ground_truth_image = copy.copy(row["tgt_img"])
                ...
                self.image_feat.append(img_slice.detach())
                self.image_label.append(torch.tensor(label_idx))
            ...
            if "audio" in self.modality:
                ...
                if 20 < audio_slice.shape[0]:
                    audio_slice = audio_slice[:20]
                self.audio.append(audio_slice)

            if "video" in self.modality:
                video_dir = f'{self.args_config.data_path}/{self.args_config.data_name}/video/'
                self.video.append(
                    torch.from_numpy(np.load(os.path.join(video_dir, row["vid"] + '.npy'), encoding='bytes')))
    ...
    def __getitem__(self, i):
        ...
        if "image" in self.outputs_modal:
            inputs_1["image_label"] = self.image_label[i]
            inputs_1["image_feat"] = self.image_feat[i]
            
        if "image_ind" in self.outputs_modal and 'summary' not in self.outputs_modal:
            # print('image_ind')
            inputs_1['tgt_Caption'] = self.label_tokenizer(
                self.img_ind[i],
                truncation=True,
                padding="max_length",
                max_length=256,
            ...
            inputs_1['summary'] = self.img_ind[i]


        if "image_ind" in self.outputs_modal and 'summary' in self.outputs_modal:
            # print('caption+image_ind')
            inputs_1['tgt_Caption'] = self.label_tokenizer(
                self.captions_img_ind[i],
                truncation=True,
                padding="max_length",
                max_length=256,
                return_tensors="pt",
            )
            inputs_1['summary'] = self.captions_img_ind[i]

        if "image_ind" not in self.outputs_modal and 'summary' in self.outputs_modal:
            ...
            inputs_1['summary'] = self.summary[i]


        # if 'src_text' in self.inputs_modal:
        inputs_1['src_Text'] = self.label_tokenizer(
            self.text[i],
            truncation=True,
            max_length=512,
            return_tensors="pt",
        ...
        inputs_1['src_text'] = self.text[i]
        
        inputs_1['gold_figs'] = self.gold_figs[i]
